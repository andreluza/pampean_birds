...
df_fss<-lapply (seq(1,length(list_sp)), function (i)
  ...
  data.frame (species = list_sp[i],
              p=res_mod_det_turno[[i]]$sims.list$mean.p,
              psi=res_mod_det_turno[[i]]$sims.list$mean.psi
              #fss=res_mod_det_turno[[i]]$sims.list$fss
  ))
...
pred_data_turno <- lapply (seq (1,length(list_sp)), function (i) {
  ...
  av_rel <-data.frame (x=c(0,1), # stand value
                       ...
                       pred= c(plogis(res_mod_det_turno[[i]]$mean$int.p),
                               plogis(res_mod_det_turno[[i]]$mean$int.p+
                                      res_mod_det_turno[[i]]$mean$alpha.turno * 1)
                               ),
                       sp=list_sp[i]
  ...
  coeff_to_pred <- data.frame (
    int=melt (res_mod_det_turno[[i]]$sims.list$int.p),
    turno=melt(res_mod_det_turno[[i]]$sims.list$alpha.turno),
    sample=rownames(melt(res_mod_det_turno[[i]]$sims.list$alpha.turno)))
  colnames(coeff_to_pred) <- c("int","turno","sample") # set colnames
  
  
  # predict for each MCMC sample
  pred_data <- lapply (seq (1,length(res_mod_det_turno[[i]]$sims.list$alpha.turno)), function (k)
